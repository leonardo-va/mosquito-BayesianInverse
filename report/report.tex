...
(the common house mosquito) and birds was conducted in \cite{Heidecke2024.09.16.613097} and \cite{Vogels2017-li}.\cite{Heidecke2024.09.16.613097} investigates the impact of temperature on some
of the model parameters. They find that a temperature of approximately 27 \textdegree C is optimal for disease transmission. The parameter values at this temperature maximize the basic reproduction number $R_0$. 
The values for the investigated parameters are given in table \ref{tab:parameters}.
\begin{table}[h!]
\centering
...
\hline
\end{tabular}
\caption{Parameter values for West Nile Virus transmission between Culex pipiens and birds at 27\textdegree C. Rates are in days$^{-1}$}
...
The term $-\alpha J^2$ in the second equation models competition between juveniles. The parameter $\alpha$ is unknown. To understand its impact on the mosquito population, and possibly figure out a sensible prior, the ODE system is simulated on the time interval $[0,100]$ (days), with different values for $\alpha$. The other parameters take the values from table \ref{tab:parameters}.


We can observe in figure \ref{mosquitoPopulation}, that the system goes into a steady state. For values $\alpha \approx 2.5*10^{-6}$ and below, after an initial jump, the populations remain relatively constant over time. 
For $\alpha = 2*10^{-6}$, the population grows slightly over the initial 100 days, but it seems that $\alpha$ acts mostly as a scaling factor for the steady state. 
For the choice $\alpha = 2*10^{-6}$ (slightly increasing mosquito population), the individual SEIR compartments for both populations can be seen in figure \ref{fig:sircompartments}.
Starting from a partially infected mosquito population and a completely susceptible host population, we can observe a wave of infections. After this wave the disease dies out, because the host population becomes immune in this model.


\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{MosquitoPop.png}
    \caption{Mosquito population over 100 days, for six different values of $\alpha$ together with the parameter values from table \ref{tab:parameters}}
...
\begin{figure}[H]
    ...
    \begin{subfigure}{0.45\textwidth}
        \centering
        \includegraphics[width=\linewidth]{SIR_Mosquitoes.png} % First plot
        \caption{}
        \label{fig:plot1}
    \end{subfigure}%
    ...
    \begin{subfigure}{0.45\textwidth}
        \centering
        \includegraphics[width=\linewidth]{SIR_Hosts.png} % Second plot
        \caption{}
        \label{fig:plot2}
    \end{subfigure}
    \caption{SEIR compartments for hosts and mosquitoes, with $\alpha = 2*10^{-6}$}
    \label{fig:sircompartments}
\end{figure}
...
In practice, not all of the compartments in the model can be measured. It is possible to obtain some very noisy measurements of the total number of adult mosquitoes in an area, and to estimate the number of juveniles in water reservoirs.
...
Gaussian noise is added to both quantities. The population sizes in this experiment are not normalized, and are on the order of thousands and tens of thousands. Thus a large standard deviation of $\sigma_M = 10000$ is chosen for the noise
in the adult population, and $\sigma_J = 20000$ for the juvenile population. This is approximately 15 \% of the signal. 
The artificial data (see figure \ref{fig:artificialdata}) is generated according to the following distribution:
\[y_i \sim \mathcal{N}\left(\boldsymbol{\begin{bmatrix}
M_i \\
J_i 
\end{bmatrix}}, 
...
\begin{figure}[H]
    ...
    \begin{subfigure}{0.45\textwidth}
        \centering
        \includegraphics[width=\linewidth]{data_total_mosquitoes.png} % First plot
        \caption{}
        \label{}
    \end{subfigure}%
    \hspace{0.05\textwidth}
    \begin{subfigure}{0.45\textwidth}
        \centering
        \includegraphics[width=\linewidth]{data_juveniles.png} % Second plot
        \caption{}
        \label{}
    \end{subfigure}
    \caption{Artificial data generated for the the total amount of adult mosquitoes and the number of juveniles, with gaussian noise.}
    \label{fig:artificialdata}
\end{figure}

Next, the goal is to infer a subset of unknown parameters $\theta_u$ from this data. The other parameters in \ref{tab:parameters} are assumed to be known, with the values given there. 
Two different sets of unknowns are considered in two experiments:
\[\theta_{u_1}:=\begin{bmatrix}\alpha \\ \mu_M\end{bmatrix}\] 
\[\theta_{u_2}:=\begin{bmatrix}\alpha \\ \mu_M \\ a \\ \delta_E\end{bmatrix}\]
The first set contains two parameters, the juvenile competition and the mosquito mortality rate. In the second set, the biting rate $a$ and the egg-to-juvenile rate $delta_E$ are added.
...
\begin{enumerate}
\item A parameter-to-observable map. Here, this is the map $\theta_u\rightarrow \ \begin{bmatrix}M\\J\end{bmatrix}$, which includes the ODE and the solver, as described above.
\item A prior distribution for the parameters. $\mathcal{N}(0.5,0.5)$ is used for every unknown parameter.
\item The observed data $y_i$ and its likelihood $p(y_i|\theta_u)$, , $0 \leq i \leq k$.
\end{enumerate}
It outputs samples for the unknowns $\theta_u$ distributed according to the posterior, and some summary statistics, as well as some information about the sampling process, like acceptance probabilities for each sample. The output can be converted to a pandas data-frame, and stored in a csv file.

\subsection{Results}
The experiment is conducted as described above. We choose the number of measurements $k=30$. For the time interval $[t_0,t_N]=[0,100]$, this would in practice mean approximately two measurements of M and J weekly. Then, 8000 samples of $\theta_u$ are generated with the No-U-Turns method. 
A kernel density estimate of the posterior of $\theta_{u_1}=\begin{bmatrix}\alpha \\ \mu_M\end{bmatrix}$ and $\theta_{u_2}=\begin{bmatrix}\alpha \\ \mu_M \\ a \\ \delta_E\end{bmatrix}$ are visualized in figure \ref{fig:posterior_2_param} figure \ref{fig:posterior_4_param} respectively. 
\begin{figure}
    \hspace*{-3cm}
    \includegraphics[width=1.4\textwidth, height=0.8\textwidth]{posterior_2.png}
    \caption{Kernel density estimate for the posteriors of $\alpha$ and $\mu_M$, calculated from samples of $\theta_{u_1}$. The green lines are the true values used for generating the artificial data, the red lines are the posterior means, and the blue ones are the map points.}
    \label{fig:posterior_2_param}
\end{figure}
\begin{figure}
    \hspace*{-3cm}
    \includegraphics[width=1.4\textwidth, height=0.8\textwidth]{posteriors_4.png}
    \caption{Kernel density estimate for the posteriors of $\alpha$, $\mu_M$, a, and $\delta_E$, calculated from samples of $\theta_{u_2}$. The green lines are the true values used for generating the artificial data, the red lines are the posterior means, and the blue ones are the map points.}
    \label{fig:posterior_4_param}
\end{figure}

The green lines show the true parameters used to generate the artificial data. We can observe that, with the exception of the parameter a, the posteriors are concentrated around the true values.
The priors were relatively flat compared to these distributions, thus it can be concluded that the measured data is informative and has a large impact on the posteriors.
For the biting rate a this is not the case. That is expected, since the measured quantities are the total adult and juvenile mosquito populations, which are unrelated to a. The biting rate only influences
the dynamics between the susceptible and exposed compartments, which are not measured individually. 


We use the mean and map of the posteriors of $\theta_{u_1}$ and $\theta_{u_2}$ and simulate the disease model once more, now with these inferred values. The other model parameters keep their values from table \ref{tab:parameters}.
The mean and map estimates are:
\[\text{mean:   } \hat{\theta}_{u_1} = \begin{bmatrix}1.974\text{e-}6 \\ 0.0996\end{bmatrix}\quad\text{map: }\hat{\theta}_{u_1} = \begin{bmatrix}1.94\text{e-}6 \\ 0.0995\end{bmatrix}\]
\[\text{mean:   } \hat{\theta}_{u_2} = \begin{bmatrix}2.297\text{e-}6 \\ 0.0984 \\ 0.4936 \\ 0.6929\end{bmatrix}\quad\text{map: }\hat{\theta}_{u_2} = \begin{bmatrix}2.384\text{e-}6 \\ 0.0979 \\ 0.4765 \\ 0.6718\end{bmatrix}\]
The resulting mosquito population $M$ and juvenile population $J$ over time are visualized in figures \ref{fig:predictions_2} and \ref{fig:predictions_4}. 
The green line is the artificial data, and the red line is the prediction using the mean/map estimators. 

To determine the quality of the samples, the effective sample size (ess) is calculated. This value describes how many samples can be considered independent,
out of the total amount. We can also estimate the cost per independent sample by dividing the runtime of the sampler by the ess.
A summary of this analysis is shown in figures \ref{fig:analysis_2} and \ref{fig:analysis_4}. In case of the two dimensional parameter space
($\theta_{u_1}$), approximately $20\%$ of the samples can be considered independent. This decreases to $6-12\%$ for the four dimensional parameter space. 
Also, the cost per independent sample increases by a factor of 10, going from two to four dimensions. 

Correlations between the scalar parameters in $\hat{\theta}_{u_1}$ and $\hat{\theta}_{u_2}$ are shown in figures 
\ref{fig:corr_2} and \ref{fig:corr_4}. We can for example see a negative correlation between $\mu_M$ and $a$ in both experiments, since
these parameters both affect the juvenile and adult mosquito populations, but in opposite directions. 
...
\begin{figure}[]
    ...
    \begin{subfigure}{0.45\textwidth}
        \centering
        \includegraphics[width=\linewidth]{adult_mean_pred.png} % First plot
        \caption{}
        \label{}
    \end{subfigure}%
    \hspace{0.05\textwidth}
    \begin{subfigure}{0.45\textwidth}
        \centering
        \includegraphics[width=\linewidth]{juvenile_mean_pred.png} % Second plot
        \caption{}
        \label{}
    \end{subfigure}
     \begin{subfigure}{0.45\textwidth}
        \centering
        \includegraphics[width=\linewidth]{adult_map_pred.png} % Second plot
        \caption{}
        \label{}
    \end{subfigure}
     \begin{subfigure}{0.45\textwidth}
        \centering
        \includegraphics[width=\linewidth]{juveniles_map_pred.png} % Second plot
        \caption{}
        \label{}
    \end{subfigure}
    \caption{Prediction of adult and juvenile populations, using the mean and map estimators for $\theta_{u_1}$.}
...
\begin{figure}[]
    ...
    \begin{subfigure}{0.45\textwidth}
        \centering
        \includegraphics[width=\linewidth]{adult_prediction_mean_4.png} % First plot
        \caption{}
        \label{}
    \end{subfigure}%
    \hspace{0.05\textwidth}
    \begin{subfigure}{0.45\textwidth}
        \centering
        \includegraphics[width=\linewidth]{juv_prediction_mean_4.png} % Second plot
        \caption{}
        \label{}
    \end{subfigure}
     \begin{subfigure}{0.45\textwidth}
        \centering
        \includegraphics[width=\linewidth]{adult_prediction_map_4.png} % Second plot
        \caption{}
        \label{}
    \end{subfigure}
     \begin{subfigure}{0.45\textwidth}
        \centering
        \includegraphics[width=\linewidth]{juv_prediction_map_4.png} % Second plot
        \caption{}
        \label{}
    \end{subfigure}
    \caption{Prediction of adult and juvenile populations, using the mean and map estimators for $\theta_{u_2}$.}
    \label{fig:predictions_4}
\end{figure}

\begin{figure}
    \centering
    \includegraphics[width=1.2\textwidth, height=0.8\textwidth]{analytics_2.png}
    \caption{Analysis of sample quality for $\theta_{u_1}$ (two dimensional parameter space). The runtime of the sampler is measured and the ESS is estimated, using the python package arviz.
    From this, the cost of per independent sample can be calculated.}
    \label{fig:analysis_2}
\end{figure}
 
\begin{figure}
    \centering
    \includegraphics[width=1.2\textwidth, height=0.8\textwidth]{analytics_4.png}
    \caption{Analysis of sample quality for $\theta_{u_2}$ (four dimensional parameter space), analogous to \ref{fig:analysis_2}. }
    \label{fig:analysis_4}
\end{figure}

\begin{figure}
    \centering
    \includegraphics[width=0.9\textwidth, height=0.5\textwidth]{correlation_2.png}
    \caption{Correlation plot for $\theta_{u_1}$ ($\mu_M$ on the y-axis and $a$ on the x-axis)}
    \label{fig:corr_2}
\end{figure}
 
\begin{figure}
    \hspace{-4cm}
    \includegraphics[width=1.7\textwidth, height=0.8\textwidth]{correlation_4.png}
    \caption{Correlation plot for $\theta_{u_2}$}
    \label{fig:corr_4}
\end{figure}
